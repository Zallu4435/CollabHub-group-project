...
import MarketplaceAdminSidebar from './components/MarketplaceAdminSidebar';
import { FiBell, FiHelpCircle, FiDownload } from 'react-icons/fi';

const MarketplaceAdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);

  return (
    <div className="min-h-screen bg-white flex">
      {/* Sidebar Component */}
      <MarketplaceAdminSidebar 
      ...
      <div className="flex-1 flex flex-col min-h-screen bg-white">
        ...
        <header className="bg-white px-6 py-4 flex-shrink-0 sticky top-0 z-30 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-gray-800 mb-0.5">Marketplace Admin</h1>
              <p className="text-sm text-gray-400">E-commerce platform management and administration</p>
            </div>

            {/* Right Section - Actions & Status */}
            <div className="flex items-center gap-3">
              {/* Status Badge */}
              <div className="flex items-center gap-2 px-3 py-1.5 bg-orange-50/50 text-orange-600 rounded-lg text-sm font-medium">
                <span className="w-2 h-2 bg-orange-400 rounded-full animate-pulse"></span>
                <span>Marketplace Admin Access</span>
              </div>
        ...
        <footer className="bg-white px-6 py-3 flex-shrink-0">
          <div className="flex items-center justify-between text-xs text-gray-300">
            <p>© 2025 Marketplace Admin. All rights reserved.</p>
