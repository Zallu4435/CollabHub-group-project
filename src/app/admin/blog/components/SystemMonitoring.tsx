    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-700 mt-1">
          Monitor system health, performance, and uptime in real-time
        </p>
      </div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-blue-600">
                {cpuData[cpuData.length - 1]?.usage?.toFixed(1) || 0}%
              </p>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-bold text-purple-600">
                {memoryData[memoryData.length - 1]?.usage?.toFixed(1) || 0}%
              </p>
              <p className="text-xs text-gray-500">Current</p>
            </div>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-base font-semibold text-gray-900 flex items-center gap-2">
              <FiServer className="text-emerald-600" size={18} />
              Microservices Health
            </h3>
            <p className="text-sm text-gray-500 mt-0.5">Real-time service status monitoring</p>
          </div>
          <button className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 border border-gray-200 rounded-lg text-sm font-medium transition-all flex items-center gap-2">
            <FiRefreshCw size={14} />
            Refresh
          </button>
        </div>
            ...
            <div key={service.name} className="p-4 bg-gray-50 border border-gray-200 rounded-lg hover:border-emerald-200 hover:shadow-sm transition-all">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                  ...
                  service.status === 'degraded' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-gray-900">{service.responseTime}ms</p>
                </div>
              </div>
              <p className="text-xs text-gray-500 mt-3 flex items-center gap-1">
                <FiClock size={10} />
                {new Date(service.lastCheck).toLocaleTimeString('en-US', {
                  hour: '2-digit',
                  minute: '2-digit'
                })}
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {[
              ...
              <div key={idx}>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="text-gray-600">{storage.used} GB / {storage.total} GB</span>
                </div>
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full bg-${storage.color}-500 rounded-full transition-all`}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h3 className="text-base font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <FiDatabase className="text-emerald-600" size={18} />
            Backup Status
          </h3>
          <div className="space-y-3">
            ...
            <button className="w-full px-4 py-2.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 font-medium transition-all flex items-center justify-center gap-2">
              <FiRefreshCw size={16} />
              Run Backup Now
            </button>
          </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="space-y-2">
          {[
            { level: 'warning', message: 'High memory usage detected on web-server-02', time: '2 minutes ago', icon: <FiAlertCircle size={16} /> },
            { level: 'error', message: 'Failed to send email to user@example.com', time: '15 minutes ago', icon: <FiAlertCircle size={16} /> },
            { level: 'info', message: 'Cache invalidation completed successfully', time: '1 hour ago', icon: <FiCheckCircle size={16} /> },
          ].map((log, idx) => (
        ...
        <button className="mt-4 w-full px-4 py-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 font-medium transition-all text-sm">
          View All Logs →
        </button>
      </div>
    </div>
